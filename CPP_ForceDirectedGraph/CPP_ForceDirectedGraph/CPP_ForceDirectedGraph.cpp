...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	auto vert = generte_vertices(20);


	// Hooke-Coulomb seems to be working fine.

	// auto out_vertices = HC_Forces::calculate_forces(20, vert, 1, 1, 250); 
	// 200: 121,145 || 1250: 120,143

	// Eades is still not 100% perfect.

	// auto out_vertices = Eades_Forces::calculate_forces(20, vert, 1, 1, 100, 1);
	...
	auto out_vertices = FR_Forces::calc_forces(20, vert, 1, 1, 1, 100);
...
void coutVertices(std::vector<Vertex> vertices)
{
	for (const auto& elem : vertices)
		std::cout << elem.position_vector[0] << ";" << elem.position_vector[1] << "\n";
}
...
std::vector<Vertex> generte_vertices(int amount)
{
	...
	for (int i = 0; i < amount; i++)
	{
		...
		int maxconnections = rand() % amount / 4;
