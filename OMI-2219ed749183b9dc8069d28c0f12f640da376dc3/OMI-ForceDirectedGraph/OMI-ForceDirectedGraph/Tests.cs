...
namespace OMI_ForceDirectedGraph
{
    partial class Tests
    {
        ...
        private static double coolingStart = 5;

        // The testing ranges for each of the variables
        private static Vector rWeightRange = new Vector(0.0001, 0.2);
        private static Vector aWeightRange = new Vector(0.0001, 0.4);
        private static Vector logAWeightRange = new Vector(0.0001, 2);
        private static Vector FRWeightRange = new Vector(0.0001, 1);
        private static Vector FRConstantRange = new Vector(0.0001, 1);
        private static Vector radiusRange = new Vector(1, 3);
        ...
        public static void ConductTests()
        {
            ...
            for (int i = 0; i < maxGraphs; i++)
            {
                ...
                for (int j = 0; j < maxConstantSettings; j++)
                {
                    ...
                    for (int k = 0; k < maxConstantSettings; k++)
                    {
                        ...
                        {
                            UpdateForces(AlgorithmType.HookeCoulomb, l);
                        }
                ...
                for (int j = 0; j < maxConstantSettings; j++)
                {
                    ...
                    for (int k = 0; k < maxConstantSettings; k++)
                    {
                        ...
                        for (int l = 0; l < maxConstantSettings; l++)
                        {
                            ...
                            {
                                UpdateForces(AlgorithmType.Eades, m);
                            }
                ...
                for (int j = 0; j < maxConstantSettings; j++)
                {
                    ...
                    for (int k = 0; k < maxConstantSettings; k++)
                    {
                        ...
                        for (int l = 0; l < maxConstantSettings; l++)
                        {
                            ...
                            for (int m = 0; m < maxIterations; m++)
                            {
                                UpdateForces(AlgorithmType.FruchtRein, m);
                            }
            ...
            WriteTestResults3(); //Uncomment this line to write the test results to a file
        }
        ...
        // Main function:
        public static void UpdateForces(AlgorithmType type, int iterations)
        {
            ...
            {
                k = type == AlgorithmType.FruchtRein ? Algorithms.FruchtReinConstant(Vertices[i], Vertices, FRConstant, radius) : 0;
            ...
            if (type == AlgorithmType.FruchtRein)
            {
                double translationSize;
                Vector tempVector;
                foreach (KeyValuePair<int, Vector> kvp in forcesDict)
                {
                    tempVector = forcesDict[kvp.Key];
                    translationSize = Math.Min(tempVector.Length, Algorithms.Cooling(coolingStart, maxIterations, iterations));
                    if (tempVector.Length != 0)
                        tempVector.Normalize();
                    forcesDict[kvp.Key] = tempVector * translationSize;
                }
            }
